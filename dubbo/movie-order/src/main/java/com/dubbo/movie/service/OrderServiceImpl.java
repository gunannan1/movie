...
import com.dubbo.movie.vo.cinema.OrderQueryVO;
...
public class OrderServiceImpl implements OrderServiceAPI {
    ...
    public boolean isTrueSeats(String fieldId, String seats) {
        // 根据FieldId找到对应的座位位置图
        String seatPath = orderInfoMapper.getSeatsByFieldId(fieldId);

        // 读取位置图，判断seats是否为真
        String fileStrByAddress = FileConvertUtil.getFileStrByAddress(seatPath);
    ...
    public boolean isNotSoldSeats(String fieldId, String seats) {
        ...
        for(OrderInfo orderInfo : list){
            String[] ids = orderInfo.getSeatsIds().split(",");
            for(String id : ids){
                for(String seat : seatArrs){
    ...
    public OrderVO saveOrderInfo(
        ...
        orderInfo.setSeatsIds(soldSeats);
        orderInfo.setOrderUser(userId);
        orderInfo.setOrderPrice(totalPrice);
        orderInfo.setFilmPrice(filmPrice);
        orderInfo.setFilmId(filmId);
        orderInfo.setFieldId(fieldId);
        orderInfo.setCinemaId(cinemaId);

        Integer insert = orderInfoMapper.insert(orderInfo);
        if(insert>0){
            // 返回查询结果
            OrderVO orderVO = orderInfoMapper.getOrderInfoById(uuid);
    ...
    public Page<OrderVO> getOrderByUserId(Integer userId,Page<OrderVO> page) {
        ...
        if(userId == null){
            ...
            List<OrderVO> ordersByUserId = orderInfoMapper.getOrdersByUserId(userId,page);
            if(ordersByUserId==null && ordersByUserId.size()==0){
                ...
                Integer counts = orderInfoMapper.selectCount(entityWrapper);
                // 将结果放入Page
    ...
    public String getSoldSeatsByFieldId(Integer fieldId) {
        if(fieldId == null){
            ...
            String soldSeatsByFieldId = orderInfoMapper.getSoldSeatsByFieldId(fieldId);
    ...
    public OrderVO getOrderInfoById(String orderId) {
        ...
        OrderVO orderInfoById = orderInfoMapper.getOrderInfoById(orderId);
